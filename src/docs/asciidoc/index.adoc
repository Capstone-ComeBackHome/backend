...
operation::disease-rest-controller-test/여러_질병명으로_간략하게_질병_조회하기[snippets='http-request,request-parameters,http-response,response-fields']

==== 간략한 질병 조회 실패
diseaseNameList 파라미터 자체가 들어오지 않거나, 공백으로 들어오거나, 콤마를 기준으로 공백이 들어온 경우 실패합니다.

include::{snippets}/disease-rest-controller-test/질병명이_빈칸으로_들어온_경우_실패/response-body.adoc[]

---

=== 질병 상세 조회
==== 질병 상세 조회 성공
operation::disease-rest-controller-test/disease-id로_상세정보_찾기[snippets='http-request,request-parameters,http-response,response-fields']

==== 질병 상세 조회 실패
diseaseId 파라미터가 들어오지 않으면 실패합니다.
include::{snippets}/disease-rest-controller-test/disease-id_파라미터_없이_상세조회/response-body.adoc[]

DB에 존재하지 않은 diseaseId인 경우 실패합니다.
include::{snippets}/disease-rest-controller-test/없는_disease-id로_상세조회/response-body.adoc[]
...
==== 진단 내역 저장 실패
토큰 없이 접근하는 경우
include::{snippets}/diagnosis-rest-controller-test/토큰없이_diagnosis_저장하기_실패/response-body.adoc[]

질병명이 3가지가 안 들어온 경우
include::{snippets}/diagnosis-rest-controller-test/한가지_질병명으로_diagnosis_저장하기_실패/response-body.adoc[]

질병명이 공백으로 들어온 경우
include::{snippets}/diagnosis-rest-controller-test/세가지_공백_질병명으로_diagnosis_저장하기_실패/response-body.adoc[]

존재하지 않는 질병명인 경우
include::{snippets}/diagnosis-rest-controller-test/존재하지_않는_질병명으로_진단_저장하기_실패/response-body.adoc[]
...
include::{snippets}/diagnosis-rest-controller-test/다른_사람의_진단_내역_제거하기_실패/response-body.adoc[]

존재하지 않는 진단 내역을 제거하는 경우
include::{snippets}/diagnosis-rest-controller-test/존재하지_않는_진단_내역_제거하기_실패/response-body.adoc[]
...
==== 월별 스케줄 조회 실패
토근 없이 접근하는 경우
include::{snippets}/calendar-rest-controller-test/토근_없이_월_스케줄_요청/response-body.adoc[]

yearMonth 파라미터 없이 접근하는 경우
include::{snippets}/calendar-rest-controller-test/year-month_없이_월_스케줄_요청/response-body.adoc[]
...
==== 특정 스케줄 상세 조회 실패
토큰 없이 접근하는 경우
include::{snippets}/calendar-rest-controller-test/토큰_없이_schedule-id로_스케줄_상세_조회/response-body.adoc[]

파라미터로 scheduleId가 들어오지 않은 경우
include::{snippets}/calendar-rest-controller-test/스케줄_id_없이_스케줄_상세_조회/response-body.adoc[]

자신의 스케줄 중 없는 스케줄 Id인 경우
include::{snippets}/calendar-rest-controller-test/자신의_스케줄_중_존재하지_않는_schedule-id로_스케줄_상세_조회/response-body.adoc[]
...
==== 특정 스케줄 수정 실패
토큰 없이 접근하는 경우
include::{snippets}/calendar-rest-controller-test/토큰_없이_스케줄_수정/response-body.adoc[]

자신의 스케줄 중 존재하지 않는 scheduleId인 경우
include::{snippets}/calendar-rest-controller-test/자신의_스케줄_중_존재하지_않는_schedule-id로_스케줄_수정/response-body.adoc[]

scheduleId path 가 들어오지 않았을 경우 Path 자체가 없기 때문에 지원하지 않는 메서드 응답이 나갑니다.
include::{snippets}/calendar-rest-controller-test/스케줄_id_없이_스케줄_수정/response-body.adoc[]

증상이 1개 미만 혹은 5개 초과로 들어온 경우
include::{snippets}/calendar-rest-controller-test/나의_스케줄_수정_증상이_0개_들어온_경우_실패/response-body.adoc[]
...
==== 스케줄 삭제 실패
토근 없이 접근하는 경우
include::{snippets}/calendar-rest-controller-test/토근_없이_스케줄_삭제_실패/response-body.adoc[]

scheduleId path 가 들어오지 않았을 경우 Path 자체가 없기 때문에 지원하지 않는 메서드 응답이 나갑니다.
include::{snippets}/calendar-rest-controller-test/스케줄_id_없이_스케줄_삭제_실패/response-body.adoc[]

내 스케줄 중 존재하지 않는 scheduleId인 경우
include::{snippets}/calendar-rest-controller-test/내_스케줄_중_존재하지_않는_schedule-id안_경우_실패/response-body.adoc[]
